...
def test_doctor_runs_and_outputs_status(capsys):
    """
    Verify that doctor.py:

    1. Executes without crashing unexpectedly
    2. Exits intentionally via SystemExit
    3. Produces human-readable diagnostic output
    """

    from interpreter.core.utils.doctor import run_doctor

    # Create an isolated interpreter instance for testing
    test_interpreter = OpenInterpreter()

    # doctor.py intentionally exits with sys.exit(...)
    # after diagnostics complete, so we explicitly expect that behavior.
    with pytest.raises(SystemExit):
        run_doctor(test_interpreter)

    # Capture all terminal output generated by doctor.py
    captured_output = capsys.readouterr()

    # Combine stdout + stderr into a single searchable string
    terminal_output = captured_output.out + captured_output.err

    # Ensure doctor.py actually printed something useful
    assert terminal_output.strip() != ""

    # Verify important diagnostic sections are present
    expected_sections = [
        "Running Open Interpreter diagnostics",
        "Python",
        "Open Interpreter",
        "Doctor Summary",
    ]
    ...
    for section in expected_sections:
        assert section in terminal_output


def test_doctor_does_not_modify_interpreter_settings():
    """
    Ensure doctor.py behaves as a read-only diagnostic utility
    and does not unexpectedly mutate interpreter configuration.
    """
    ...
    original_model = test_interpreter.llm.model
    original_auto_run = test_interpreter.auto_run
    original_temperature = test_interpreter.llm.temperature

    # doctor.py intentionally terminates via sys.exit(...)
    with pytest.raises(SystemExit):
        run_doctor(test_interpreter)

    # Verify interpreter configuration was not modified
    assert test_interpreter.llm.model == original_model
    assert test_interpreter.auto_run == original_auto_run
    assert test_interpreter.llm.temperature == original_temperature


def test_doctor_handles_missing_package(monkeypatch, capsys):
    """
    Verify doctor.py gracefully handles missing optional packages
    and reports the failure in diagnostic output.
    """
    ...
    # Save the original Python import function
    original_import_function = builtins.__import__

    # Simulate a missing dependency when doctor.py attempts
    # to import the torch package.
    def mocked_import(name, *args, **kwargs):
        if name == "torch":
            raise ImportError("Mocked missing torch package")

        return original_import_function(name, *args, **kwargs)

    # Replace Python's import system temporarily
    ...
    # Capture generated terminal output
    captured_output = capsys.readouterr()
    ...
    # Ensure the missing package was reported
    assert "torch" in terminal_output

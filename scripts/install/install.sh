...
CODEX_HOME_DIR="$INTERPRETER_HOME_DIR"
...
if ! release_dir_is_complete "$release_dir" "$resolved_version" "$vendor_target" "$install_layout"; then
  echo "Installed $PRODUCT_NAME command did not report expected version $resolved_version." >&2
  exit 1
fi
if [ "$DAEMON_ONLY" = "1" ] && [ "${CODEX_INSTALL_DEFER_SELECTION:-0}" != "1" ]; then
  installed_codex="$release_dir/$COMMAND_NAME"
  if [ "$install_layout" = "package" ]; then
    installed_codex="$release_dir/$(package_entrypoint_relative_path "$release_dir")"
  fi

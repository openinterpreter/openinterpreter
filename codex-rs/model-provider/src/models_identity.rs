...
use crate::auth::provider_credential_fingerprint;
use crate::auth::resolve_provider_auth_sync;

pub(crate) fn identity(
    provider_info: &ModelProviderInfo,
    auth: Option<&CodexAuth>,
) -> CoreResult<String> {
    identity_with_credential_fingerprint(provider_info, auth, None)
}

pub(crate) fn identity_with_codex_home(
    provider_info: &ModelProviderInfo,
    auth: Option<&CodexAuth>,
...
pub(crate) fn identity_with_credential_fingerprint(
    ...
    let explicit_bearer = if credential_fingerprint.is_some() {
        false
    } else {
        provider_info.api_key()?.is_some() || provider_info.experimental_bearer_token.is_some()
    };
    if let Some(fingerprint) = credential_fingerprint {
        field(b"provider_credential_fingerprint");
    ...
    if credential_fingerprint.is_none() && (!has_stable_account || explicit_bearer) {
        let headers = resolve_provider_auth_sync(auth, provider_info)?.to_auth_headers();

...
pub(crate) fn get_tooltip(
    ...
    preferred_tooltip(&mut rng, plan, fast_mode_enabled, product).or_else(|| {
        pick_tooltip(&mut rng, keymap).map(|tooltip| tooltip_for_product(&tooltip, product))
    })
}
...
pub(crate) fn preferred_tooltip<R: Rng + ?Sized>(
    ...
    if product == Product::Codex && rng.random_ratio(/*numerator*/ 8, /*denominator*/ 10) {
        match plan {
            Some(plan_type)
                if matches!(
                    plan_type,

...
pub(crate) struct CollaborationModesConfig {
...
mod tests {
    ...
    #[test]
    fn default_preset_name_matches_mode() {
        assert_eq!(
    ...
    #[test]
    fn default_preset_includes_request_user_input_guidance_when_enabled() {
        ...
        assert!(
            default_instructions.contains(
                "prefer using the `request_user_input` tool rather than writing a multiple choice question as a textual assistant message."
            )
        );
    }

    #[test]
    fn plan_preset_matches_mode() {

...
sudo apt-get install -y "${apt_install_args[@]}" ca-certificates curl musl-tools pkg-config libcap-dev linux-libc-dev g++ clang libc++-dev libc++abi-dev lld xz-utils perl make
case "${TARGET}" in
